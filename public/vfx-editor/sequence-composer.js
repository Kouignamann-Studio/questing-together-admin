...
async function loadSpriteManifest() {
  ...
  state.spriteLibrary = await buildSpriteLibraryFromLinkedRepo(entries);
  await Promise.all(
    Object.values(state.spriteLibrary)
...
async function linkRepoRoot() {
  try {
    if (!('showDirectoryPicker' in window)) {
      setStatus('Link Game Repo Root requires Chrome, Arc, or Edge with File System Access support.', 'error');
      return;
    }
    ...
    if (!isValidRoot) {
      setStatus(
        'Choose the game repo root that contains src/features/vfx.',
        'error',
      );
      return;
    }
    ...
    setStatus('Linked the game repo and refreshed the effect list.', 'success');
  } catch (error) {
    if (error?.name === 'AbortError') return;
    setStatus(`Could not link the game repo: ${error.message}`, 'error');
  }
}
  ...
  setStatus(
    restoredSessionSource === 'browser'
      ...
      : state.repoRootHandle
        ? 'Link is active. Build a sequence using effects from the linked game repo.'
        : 'Link the game repo or open a sequence JSON to start composing.',
  );
  window.requestAnimationFrame(animationLoop);
}
