...
function clearSpritePreviewObjectUrls() {
  for (const url of spritePreviewObjectUrls) {
    URL.revokeObjectURL(url);
  }

  spritePreviewObjectUrls = [];
}
...
async function loadSpriteManifestFromRepoHandle() {
  ...
  try {
    ...
    const spritesDirHandle = await getExistingDirectoryHandle(state.repoRootHandle, REPO_SPRITES_SEGMENTS);
    ...
    console.warn('Could not load the sprite manifest from the linked game repo.', error);
    return [];
  }
}

async function buildSpriteLibraryFromLinkedRepo(entries) {
  if (!state.repoRootHandle || entries.length === 0) {
    return {};
  }
  ...
  const spriteEntries = await Promise.all(
    entries.map(async (entry) => {
      try {
        const fileHandle = await spritesDirHandle.getFileHandle(entry.filename);
        const file = await fileHandle.getFile();
        const src = URL.createObjectURL(file);
        spritePreviewObjectUrls.push(src);
        return [
          entry.id,
          {
            label: entry.label,
        ...
        console.warn(`Could not load sprite preview "${entry.filename}".`, error);
        return [
          entry.id,
          {
            label: entry.label,
  ...
  return Object.fromEntries(spriteEntries);
}

async function loadSpriteManifest() {
  clearSpritePreviewObjectUrls();
  ...
  spriteLibrary = await buildSpriteLibraryFromLinkedRepo(entries);
  void Promise.all(
    Object.values(spriteLibrary)
      .filter((sprite) => sprite.src)
      .map((sprite) => preloadSpritePreviewSrc(sprite.src)),
  );
  return entries;
}
...
async function getExistingDirectoryHandle(rootHandle, segments) {
  let current = rootHandle;
...
async function validateRepoRootHandle(rootHandle) {
  try {
    await getExistingDirectoryHandle(rootHandle, REPO_EFFECTS_SEGMENTS);
    await getExistingDirectoryHandle(rootHandle, REPO_SEQUENCES_SEGMENTS);
    await getExistingDirectoryHandle(rootHandle, REPO_SPRITES_SEGMENTS);
    await getExistingDirectoryHandle(rootHandle, REPO_RUNTIME_SEGMENTS);
    return true;
  } catch (error) {
    return false;
  }
...
async function refreshLinkedRepoPreviewCatalogs() {
  if (!state.repoRootHandle) {
    state.repoEffectEntries = [];
    state.repoSequenceEntries = [];
    return;
  }

  try {
    const [effectEntries, sequenceEntries] = await Promise.all([
      discoverRepoEffectEntries(state.repoRootHandle),
      discoverRepoSequenceEntries(state.repoRootHandle),
    ]);

    state.repoEffectEntries = effectEntries;
    state.repoSequenceEntries = sequenceEntries;
  } catch (error) {
    console.warn('Could not refresh preview catalogs from the linked game repo.', error);
    state.repoEffectEntries = [];
    state.repoSequenceEntries = [];
  }
}
...
async function importSprite() {
  try {
    ...
    if (!isValidRoot) {
      setSaveStatus('Choose the game repo root that contains src/features/vfx.', 'error');
      return;
    }
    ...
    spriteLibrary = await buildSpriteLibraryFromLinkedRepo(spriteEntries);
...
async function loadPreviewEffectAsset(assetId) {
  ...
  return loadPreviewEffectAssetFromRepoHandle(assetId);
}

async function loadPreviewEffectAssetFromRepoHandle(assetId) {
  if (!state.repoRootHandle) {
    return null;
  }

  let entry = state.repoEffectEntries.find((item) => item.id === assetId);
  if (!entry) {
    await refreshLinkedRepoPreviewCatalogs();
    entry = state.repoEffectEntries.find((item) => item.id === assetId);
  }
  ...
  try {
    const effectsDirHandle = await getExistingDirectoryHandle(state.repoRootHandle, REPO_EFFECTS_SEGMENTS);
    const fileHandle = await effectsDirHandle.getFileHandle(entry.filename);
    const file = await fileHandle.getFile();
    const asset = normalizeAsset(JSON.parse(await file.text()));
    effectPreviewLibrary[assetId] = asset;
    return asset;
  } catch (error) {
    console.warn(`Could not load preview asset "${assetId}" from the linked game repo.`, error);
    return null;
  }
}
...
function getSequenceAssetOptions() {
  const ids = new Set([
    state.asset.id,
    ...state.repoEffectEntries.map((entry) => entry.id),
...
async function loadPreviewSequence(sequenceId) {
  ...
  return loadPreviewSequenceFromRepoHandle(sequenceId);
}

async function loadPreviewSequenceFromRepoHandle(sequenceId) {
  if (!state.repoRootHandle) {
    return null;
  }

  let entry = state.repoSequenceEntries.find((item) => item.id === sequenceId);
  if (!entry) {
    await refreshLinkedRepoPreviewCatalogs();
    entry = state.repoSequenceEntries.find((item) => item.id === sequenceId);
  }
  ...
  try {
    const sequencesDirHandle = await getExistingDirectoryHandle(
      state.repoRootHandle,
      REPO_SEQUENCES_SEGMENTS,
    );
    const fileHandle = await sequencesDirHandle.getFileHandle(entry.filename);
    const file = await fileHandle.getFile();
    const sequence = normalizeSequence(JSON.parse(await file.text()));
    ...
    console.warn(`Could not load preview sequence "${sequenceId}" from the linked game repo.`, error);
    return null;
  }
}
...
const state = {
  ...
  previewSequenceId: 'starter-sequence',
  previewSequence: null,
  pendingSequenceAssetId: '',
...
function normalizePreviewSequenceId(rawValue) {
  return typeof rawValue === 'string' && rawValue.trim() ? rawValue.trim() : 'starter-sequence';
}
...
async function refreshEffectCollectionModalData({ selectedFileName = state.selectedEffectCollectionFileName } = {}) {
  ...
  if (!repoRootHandle) {
    ...
    setEffectCollectionStatus('Link the game repo root to manage saved VFX effect files.', 'info');
    return false;
  }
...
async function linkRepoRoot() {
  try {
    if (!('showDirectoryPicker' in window)) {
      setSaveStatus('Link Game Repo Root requires Chrome, Arc, or Edge with File System Access support.', 'error');
      return;
    }

    setSaveStatus('Choose the game repo root folder in the browser picker.', 'info');
    ...
    renderPanels();
    renderPreview();
    setSaveStatus('Linked the game repo. Quick load, sprite import, and registry refresh now target that repo.', 'success');
  } catch (error) {
    if (error?.name === 'AbortError') return;
    setSaveStatus(`Could not link the game repo: ${error.message}`, 'error');
  }
}
    ...
    ui.effectCollectionModalBody.innerHTML = `
      <div class="effect-collection-shell">
        <div class="effect-collection-toolbar">
          <p class="panel-note">Link the game repo root to manage saved effect files and regenerate registries safely.</p>
          <div class="button-row">
            <button class="action-button action-button-secondary" type="button" data-collection-action="link-repo-root">Link Game Repo Root</button>
          </div>
        </div>
...
function renderToolbarMenus() {
  ...
  ui.fileMenuContent.innerHTML = `
    <div class="menu-grid">
      ...
      <section class="menu-section">
        <p class="menu-section-title">Quick Load Repo Effects</p>
        ${
          state.repoEffectEntries.length
            ? `
          <div class="menu-button-stack">
            ${state.repoEffectEntries
              .map(
                (entry) => `
              <button class="menu-action" type="button" data-toolbar-action="load-demo" data-demo-effect="${escapeHtml(entry.id)}">${escapeHtml(entry.label)}</button>
            `,
              )
              .join('')}
          </div>
        `
            : '<p class="menu-section-note">Link the game repo root to browse saved effect assets.</p>'
        }
      </section>
    </div>
  `;
  ...
  ui.editorMenuContent.innerHTML = `
    <div class="menu-grid">
      <section class="menu-section">
        ...
        <div class="menu-button-stack">
          <button class="menu-action" type="button" data-toolbar-action="link-repo-root">${repoLinked ? 'Relink Game Repo Root' : 'Link Game Repo Root'}</button>
        ...
        <p class="menu-section-note">
          ${repoLinked ? 'The linked game repo is used for quick load, sprite import, and registry refresh.' : 'Link the game repo once so the tool can read and update VFX assets.'}
        </p>
      </section>
...
async function renameSelectedEffectCollectionEntry() {
  ...
  try {
    ...
    const repoRootHandle = await resolveRepoRootHandleForRegistryRefresh();
    if (!repoRootHandle) {
      throw new Error('Link the game repo root first.');
    }
...
async function loadDemoEffect(effectId) {
  const asset = await loadPreviewEffectAssetFromRepoHandle(effectId);
  if (!asset) {
    setSaveStatus(
      `Could not load ${effectId}. Link the game repo root and make sure the asset exists in src/features/vfx/assets/effects.`,
      'error',
    );
    return;
  }

  const entry = state.repoEffectEntries.find((item) => item.id === effectId);
  loadAssetFromObject(asset, { fileName: entry?.filename ?? `${effectId}.json` });
}
    ...
    if (state.fileHandle && 'createWritable' in state.fileHandle) {
      ...
      setSaveStatus(
        result.registryError
          ...
          ? `Saved ${state.fileName}, but effectRegistry.ts was not refreshed because the game repo is not linked. Use Editor > Link Game Repo Root once, then save again.`
    ...
    if (state.sequenceFileHandle && 'createWritable' in state.sequenceFileHandle) {
      ...
      setSaveStatus(
        result.registryError
          ...
          : result.registryNeedsRepoLink
          ? `Saved sequence ${state.sequenceFileName}, but sequenceRegistry.ts was not refreshed because the game repo is not linked. Use Editor > Link Game Repo Root once, then save again.`
          : result.registryUpdated === false
...
function resetEditorStateForRecovery() {
  ...
  state.previewViewport = createDefaultPreviewViewport();
  state.previewPlaybackMode = 'single';
  state.previewSequenceId = 'starter-sequence';
  state.previewSequence = createSequenceFromCurrentAsset();
  state.previewSequenceId = state.previewSequence.id;
  state.pendingSequenceAssetId = state.asset.id;
  state.selectedSequenceCueId = '';
  ...
  try {
    ...
    setSaveStatus(
      restoredSessionSource === 'browser'
        ...
        : state.repoRootHandle
          ? 'Link is active. Open a JSON file or quick-load an effect from the linked game repo.'
          : 'Open a JSON file or link the game repo to browse saved effect assets.',
    );
  } catch (error) {
